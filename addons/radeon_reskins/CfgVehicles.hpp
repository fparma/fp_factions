class C_PilotJacket_black_UniformHolder_RF;
class C_PilotJacket_Open_black_UniformHolder_RF;
class JAM_U_Obr88_gry_Unit;
class FP_U_pilotjacket_drive_Unit: C_PilotJacket_black_UniformHolder_RF {
    uniformClass = "FP_U_pilotjacket_drive";
    hiddenSelectionsTextures[] = {"\FP_Factions\radeon_reskins\SkinsUniform\jacket_drive_co.paa","\a3\characters_f_aow\Uniforms\Data\U_C_ArtTShirt_01_v1_F_co.paa"};
};
class FP_U_pilotjacket_open_drive_Unit: C_PilotJacket_Open_black_UniformHolder_RF {
    uniformClass = "FP_U_pilotjacket_open_drive";
    hiddenSelectionsTextures[] = {"\FP_Factions\radeon_reskins\SkinsUniform\jacket_drive_co.paa","\a3\characters_f_aow\Uniforms\Data\U_C_ArtTShirt_01_v1_F_co.paa"};
};
class FP_U_Obr88_hex_Unit: JAM_U_Obr88_gry_Unit {
    uniformClass = "FP_U_Obr88_hex";
    hiddenSelectionsTextures[] = {"\FP_Factions\radeon_reskins\SkinsUniform\jam_obr88_hex_vanilla_co.paa"};
};
class FP_U_Obr88_rolled_hex_Unit: JAM_U_Obr88_gry_Unit {
    uniformClass = "FP_U_Obr88_rolled_hex";
    hiddenSelectionsTextures[] = {"\FP_Factions\radeon_reskins\SkinsUniform\jam_obr88_hex_vanilla_co.paa"};
};
class FP_U_Obr88_ghex_Unit: JAM_U_Obr88_gry_Unit {
    uniformClass = "FP_U_Obr88_ghex";
    hiddenSelectionsTextures[] = {"\FP_Factions\radeon_reskins\SkinsUniform\jam_obr88_hex_tropic_co.paa"};
};
class FP_U_Obr88_rolled_ghex_Unit: JAM_U_Obr88_gry_Unit {
    uniformClass = "FP_U_Obr88_rolled_ghex";
    hiddenSelectionsTextures[] = {"\FP_Factions\radeon_reskins\SkinsUniform\jam_obr88_hex_tropic_co.paa"};
};
class FP_U_Obr88_hex_semi_Unit: JAM_U_Obr88_gry_Unit {
    uniformClass = "FP_U_Obr88_hex_semi";
    hiddenSelectionsTextures[] = {"\FP_Factions\radeon_reskins\SkinsUniform\jam_obr88_hex_semi_arid_co.paa"};
};
class FP_U_Obr88_rolled_hex_semi_Unit: JAM_U_Obr88_gry_Unit {
    uniformClass = "FP_U_Obr88_rolled_hex_semi";
    hiddenSelectionsTextures[] = {"\FP_Factions\radeon_reskins\SkinsUniform\jam_obr88_hex_semi_arid_co.paa"};
};
class FP_U_Obr88_hex_arid_Unit: JAM_U_Obr88_gry_Unit {
    uniformClass = "FP_U_Obr88_hex_arid";
    hiddenSelectionsTextures[] = {"\FP_Factions\radeon_reskins\SkinsUniform\jam_obr88_hex_arid_co.paa"};
};
class FP_U_Obr88_rolled_hex_arid_Unit: JAM_U_Obr88_gry_Unit {
    uniformClass = "FP_U_Obr88_rolled_hex_arid";
    hiddenSelectionsTextures[] = {"\FP_Factions\radeon_reskins\SkinsUniform\jam_obr88_hex_arid_co.paa"};
};
class FP_U_Obr88_hex_desert_Unit: JAM_U_Obr88_gry_Unit {
    uniformClass = "FP_U_Obr88_hex_desert";
    hiddenSelectionsTextures[] = {"\FP_Factions\radeon_reskins\SkinsUniform\jam_obr88_hex_desert_co.paa"};
};
class FP_U_Obr88_rolled_hex_desert_Unit: JAM_U_Obr88_gry_Unit {
    uniformClass = "FP_U_Obr88_rolled_hex_desert";
    hiddenSelectionsTextures[] = {"\FP_Factions\radeon_reskins\SkinsUniform\jam_obr88_hex_desert_co.paa"};
};
class FP_U_Obr88_hex_naval_Unit: JAM_U_Obr88_gry_Unit {
    uniformClass = "FP_U_Obr88_hex_naval";
    hiddenSelectionsTextures[] = {"\FP_Factions\radeon_reskins\SkinsUniform\jam_obr88_hex_oceanic_co.paa"};
};
class FP_U_Obr88_rolled_hex_naval_Unit: JAM_U_Obr88_gry_Unit {
    uniformClass = "FP_U_Obr88_rolled_hex_naval";
    hiddenSelectionsTextures[] = {"\FP_Factions\radeon_reskins\SkinsUniform\jam_obr88_hex_oceanic_co.paa"};
};
class FP_U_Obr88_hex_urban_Unit: JAM_U_Obr88_gry_Unit {
    uniformClass = "FP_U_Obr88_hex_urban";
    hiddenSelectionsTextures[] = {"\FP_Factions\radeon_reskins\SkinsUniform\jam_obr88_hex_urban_co.paa"};
};
class FP_U_Obr88_rolled_hex_urban_Unit: JAM_U_Obr88_gry_Unit {
    uniformClass = "FP_U_Obr88_rolled_hex_urban";
    hiddenSelectionsTextures[] = {"\FP_Factions\radeon_reskins\SkinsUniform\jam_obr88_hex_urban_co.paa"};
};